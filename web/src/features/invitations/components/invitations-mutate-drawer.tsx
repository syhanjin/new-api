...
export function InvitationsMutateDrawer({ open, onOpenChange, currentRow }: { open: boolean; onOpenChange: (open: boolean) => void; currentRow?: InvitationCode }) {
  const { t } = useTranslation(); const { triggerRefresh, setResult, setOpen } = useInvitations(); const [loading, setLoading] = useState(false); const form = useForm<Values>({ resolver: zodResolver(schema), defaultValues: { name: '', count: 1, max_uses: 1 } })
  ...
  async function submit(values: Values) {
    setLoading(true)
    try {
      const payload = { ...values, expired_time: values.expired_time ? Math.floor(values.expired_time.getTime() / 1000) : 0 }
      if (currentRow) {
        ...
        toast.success(t(INVITATION_MESSAGES.CREATED))
        onOpenChange(false)
        triggerRefresh()
        if (result.data?.codes) {
          setResult({ batchName: result.data.batch.name, codes: result.data.codes })
