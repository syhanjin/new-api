...
export function InvitationsDialogs() {
  const { open, setOpen, currentRow } = useInvitations()
  const update = open === 'update'
  return <><InvitationsMutateDrawer open={open === 'create' || update} onOpenChange={(value) => !value && setOpen(null)} currentRow={update ? currentRow || undefined : undefined} /><InvitationsDeleteDialog /><InvitationsCreateResultDialog /></>
}
